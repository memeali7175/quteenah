...
const handler = async (m, { command, usedPrefix, conn, text }) => {
  ...
  if (command === 'تنزيل' && (device == 'desktop' || device == 'web')) throw `*[❗] Los mensajes de botones aun no estan disponibles en WhatsApp web, acceda a su celular para poder ver y usar los mensajes con botones.*`;
  if (enviando) return;
  enviando = true;
  ...
  try {
    ...
    if (!text.includes('SN@') && command !== 'تنزيل') await conn.sendMessage(m.chat, { text: dataMessage }, { quoted: m });      
                          ...
                          buttons: [
                              {
                                  ...
                                  buttonParamsJson: JSON.stringify({
                                      display_text: 'صوت',
                                      id: `${usedPrefix}تشغيل1 ${data.resultado.url} SN@`
                                  })
                              },
                              {
                                  name: 'quick_reply',
                                  buttonParamsJson: JSON.stringify({
                                      display_text: 'فيديو',
                                      id: `${usedPrefix}تشغيل2 ${data.resultado.url} SN@`
                                  })
                              },   
                          ],
                          messageParamsJson: "",
...
handler.command = /^(تشغيل1|تشغيل2|تنزيل)$/i;
