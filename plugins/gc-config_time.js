...
${tradutor.texto1[1]} *${usedPrefix + command} قفل 1* 
${tradutor.texto1[2]}
`;
    m.reply(caption);
	  throw false;
  ...
  await conn.groupSettingUpdate(m.chat, isClose).then(async (_)=> {
	  m.reply(`⚠️🚫 *_الجـروب ${isClose == 'announcement' ? 'مغلق' : 'مفتوح'} ${args[1] ? `خلال *${clockString(timeoutset)}_*` : ''}`);
  });
  if (args[1]) {
	 setTimeout(async () => {
...
handler.tags = ['group'];
handler.command = /^(جروبتايم|gctime)$/i;
