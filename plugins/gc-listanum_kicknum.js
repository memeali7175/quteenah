...
const handler = async (m, {conn, args, groupMetadata, participants, usedPrefix, command, isBotAdmin, isSuperAdmin}) => {
  ...
  if (!args[0]) return m.reply(`${tradutor.texto1} ${usedPrefix + command} 966*`);
  if (isNaN(args[0])) return m.reply(`${tradutor.texto2} ${usedPrefix + command} 966*`);
...
handler.command = /^(listanum|كشف|listnum)$/i;
