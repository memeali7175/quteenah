...
const handler = async (m, {conn, usedPrefix, text}) => {
  ...
  if (!text && !m.quoted) return conn.reply(m.chat, `${tradutor.texto1[0]} ${usedPrefix}خفض @تاغ*\n*┠≽ ${usedPrefix}خفض ${tradutor.texto1[1]}`, m);
...
handler.tags = ['group'];
handler.command = /^(demote|خفض|quitaradmin)$/i;
handler.group = true;
handler.admin = true;
